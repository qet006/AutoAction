...
#         return;
    ...
    if obj.text == '打卡签到':
        ...
        if obj.text == "签到完毕":
            log_print(
                "---签到完毕*****"+browser.find_element_by_css_selector('#my-balance span').text, 1)
        else:
...
def denglu():
    ...
    while browser.current_url.find('wp-login.php') > -1:
        ...
        t1 = zh(browser.find_element_by_css_selector(
            '.cptch_span:nth-child(1)').text)
        t2 = browser.find_element_by_css_selector(
            '.cptch_span:nth-child(2)').text
        t3 = zh(browser.find_element_by_css_selector(
            '.cptch_span:nth-child(3)').text)
        t5 = zh(browser.find_element_by_css_selector(
            '.cptch_span:nth-child(5)').text)
        while (t1 == '' and t3 == '') or (t1 == '' and t5 == '') or (t3 == '' and t5 == ''):
            browser.find_element_by_css_selector(
                '.cptch_reload_button').click()
            time.sleep(2)
            t1 = zh(browser.find_element_by_css_selector(
                '.cptch_span:nth-child(1)').text)
            t2 = browser.find_element_by_css_selector(
        ...
        browser.find_element_by_css_selector(
            '.cptch_wp_login').send_keys(zhfh(t1, t2, t3, t5))
...
def zhfh(t1, t2, t3, t5):
    if t2 == ' + ':
        if t1 == '':
            return str(int(t5-t3))
        if t3 == '':
            return str(int(t5-t1))
        if t5 == '':
            return str(int(t3+t1))
    if t2 == ' − ':
        if t1 == '':
            return str(int(t5+t3))
        if t3 == '':
            return str(int(t1-t5))
        if t5 == '':
            return str(int(t1-t3))
    if t2 == ' × ':
        if t1 == '':
            return str(int(t5/t3))
        if t3 == '':
...
def ekp_qd():
    ...
    log_print("username: "+username+"  password: "+password)
    log_print("当前网址:"+browser.current_url)
    time.sleep(3)
    browser.find_element_by_css_selector(
        '.lui_login_input_username').send_keys(username)
    browser.find_element_by_css_selector(
        '.lui_login_input_password').send_keys(password)
